# cosmic-debian
Basic scripts I use to build COSMIC Epoch DE on Debian Trixie. The `install-build.sh` installs the build requirements, some may not be needed, but I wanted to make sure I had everything needed before starting. The `find-depends.sh` I will use when changes are made to the cosmic-epoch debian/control files to see if my scripts may need updating. The `build-cosmic.sh` is the script that actually builds the packages.
...
```
sudo apt install cosmic-initial-setup cosmic-session cosmic-trixie-addons
```
